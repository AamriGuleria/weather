...
const fetchData=async (target) => {
    ...
    console.log(aamri);
    const {
        main:{temp},weather:{0:{id,main,desciption,icon}}}=aamri;
        update(temp,aamri.name ,icon,main,aamri.dt,aamri.timezone);
} catch(error){
    alert("Location not found");
}
};
function update(temperate,city,icon,main,dt,tz){
